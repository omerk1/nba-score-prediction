...
- **Target reformulation** — executed. See the `target_formulation_diff_total` decision-log entry below (section 4): implemented as `model.target_formulation` config flag (`ScorePredictor._to_training_targets`/`_from_training_targets`), full 5-fold CV comparison run. **Result: `diff_total` beats `home_away` on val_score in all 5 folds, mean 1.3838 vs. 1.3851 — the first candidate in this project's CV-protocol history to clearly clear the per-fold guardrail. Not yet adopted; flagged for review before flipping the config default**, since it changes the training loss (out of autonomous-execution scope per the guardrail, and consequential enough to want explicit confirmation regardless of who's running it).
...
- Conclusion: **this is the first candidate in this project's CV-protocol history to cleanly clear the per-fold guardrail** — unanimous improvement across all 5 folds, full CV, not a fold1-driven artifact. Magnitude is modest (0.0013 on val_score_mean) but real and consistent. **Not adopted — flagged for review before flipping `target_formulation` to `diff_total`** in `configs/config.yaml`, since it changes the training loss (this project's own convention treats that as needing explicit sign-off regardless of who's running the experiment).
- Next: pending review. If adopted: flip `configs/config.yaml`'s `model.target_formulation` to `diff_total`, re-log as the new champion row. If not: config flag stays in place (disabled-from-adoption) for future revisit.
