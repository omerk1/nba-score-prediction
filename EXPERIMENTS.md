...
**Champion baseline** (`champion_cv_baseline_lambda_0.75`, `outputs/experiments_v2.csv`): `style_matchup.raw_features_enabled=true`, `preferred_opponent_delta_enabled=true`, `style_matchup.enabled=false`, `model.target_formulation=diff_total`, `model.target_lambda_weight=0.75`.
...
- **ADOPTED.** `configs/config.yaml`'s `model.target_lambda_weight` flipped to `0.75` — new standing champion, logged as `champion_cv_baseline_lambda_0.75` (fresh `run_name`, same convention as the prior two supersessions). Values byte-identical to `target_lambda_weight_0.75`'s row, not re-executed — CV determinism was already proven for this code path (`target_formulation_diff_total`'s determinism entry above), and this run used the identical harness.
- Next: none — closed.
