...
**Champion baseline** (`target_formulation_diff_total`, `outputs/experiments_v2.csv`): `style_matchup.raw_features_enabled=true`, `preferred_opponent_delta_enabled=true`, `style_matchup.enabled=false`, `model.target_formulation=diff_total`.
...
- **ADOPTED.** `configs/config.yaml`'s `model.target_formulation` flipped to `diff_total` — new standing champion. No re-run needed for the champion designation: `diff_total` mode's determinism is proven, so `target_formulation_diff_total`'s already-logged row *is* what any future run of this exact config would produce.
