...
def main() -> None:
    ...
    parser.add_argument(
        "--n-trials", type=int, default=None, help="Override config elo_features.tuning.n_trials"
    )
    ...
    all_games = loader.load_games(
        ...
        allowed_season_types=config.datasets_loading.context_season_types
        or config.datasets_loading.allowed_season_types,
    )
    loader.close()
    ...
    logger.info(
        "Default Elo params (k=20, home_adv=100, mov=True, season_reg=0.33) gave val_diff_mae=11.050 (elo_v1)"
    )
