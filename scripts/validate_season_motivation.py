...
Produces:
  ...
  2. Sanity checks printed to stdout: no NaN, motivation_score in [0,1],
...
MOTIVATION_COLS = [
    "home_team_motivation_score", "home_team_games_to_clinch_ceiling", "home_team_games_to_clinch_floor",
    "away_team_motivation_score", "away_team_games_to_clinch_ceiling", "away_team_games_to_clinch_floor",
]
...
def main():
    ...
    score_cols = ["home_team_motivation_score", "away_team_motivation_score"]
    out_of_range = feats[
        (feats[score_cols] < 0).any(axis=1) | (feats[score_cols] > 1).any(axis=1)
    ]
    logger.info(f"Sanity check 2 -- motivation_score outside [0,1]: {len(out_of_range)} / {len(feats)}")
    ...
    logger.info(
        "Sanity check 4 -- home_team_motivation_score distribution: "
        f"min={feats['home_team_motivation_score'].min():.3f}, "
        f"mean={feats['home_team_motivation_score'].mean():.3f}, "
        f"max={feats['home_team_motivation_score'].max():.3f}"
    )
    locked_both = (
