...
**Update (2026-09-21):** the report-date finding above is fixed deterministically, along with two silent row-loss defects found by the extraction telemetry added alongside it. Rows recovered across the 814 reports went from 76,778 to 96,950. See `docs/features/injury_pdf_extraction_scope.md`'s phase A results. `_add_injury_features` still reads the old report-date-keyed table; repointing it is gated behind the injury ablation.
