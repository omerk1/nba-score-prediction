...
own `_get_importance_map` formula. `motivation_score` (a single hand-combined
score) was retired in favor of exposing `standings_pressure` and
`roster_behavior_score` as separate raw columns -- see feature_builder.py's
docstring for why. These tests lock in:
  ...
  4. standings_pressure decays toward 0 for a team far from the line with few
     ...
     designed to produce (this is the property that lets a team either
     "comfortably clinched" or "hopelessly eliminated" read as low-pressure).
  ...
  8. Home-away diff columns are computed correctly for all four raw metrics.
  9. season_motivation.enabled=False short-circuits before touching any DB.
  10. Missing injury features DB soft-disables (warn + return df unchanged).
"""
...
def _mock_config(raw_db_path, injury_db_path, enabled: bool = True, playoff_line_seed: int = 10, min_importance_games: int = 5):
    mock_cfg = MagicMock()
    mock_cfg.data_paths = MagicMock(raw_db=str(raw_db_path))
    mock_cfg.season_motivation = MagicMock(enabled=enabled, playoff_line_seed=playoff_line_seed, min_importance_games=min_importance_games)
...
class TestAddSeasonMotivationFeatures:
    ...
    @patch("src.feature_engineering.feature_builder.load_config")
    def test_diff_columns_computed_correctly(self, mock_config, tmp_path):
        """home_team_* minus away_team_* for all four raw metrics, exactly."""
        raw_db = tmp_path / "nba_api.sqlite"
        ...
        for metric in ["standings_pressure", "roster_behavior_score", "games_to_clinch_ceiling", "games_to_clinch_floor"]:
            expected = result.loc[0, f"home_team_{metric}"] - result.loc[0, f"away_team_{metric}"]
            assert result.loc[0, f"{metric}_diff"] == pytest.approx(expected)

    @patch("src.feature_engineering.feature_builder.load_config")
    def test_no_motivation_score_column_present(self, mock_config, tmp_path):
        """The old combined motivation_score column must no longer exist --
        replaced by standings_pressure/roster_behavior_score as separate raw
        columns (see feature_builder.py's docstring for why)."""
        raw_db = tmp_path / "nba_api.sqlite"
        injury_db = tmp_path / "injury_features.sqlite"
        _write_game_db(raw_db, _round_robin_games())
        _write_injury_features_db(injury_db, importance_rows=[], injury_rows=[])
        ...
        assert "home_team_motivation_score" not in result.columns
        assert "away_team_motivation_score" not in result.columns
