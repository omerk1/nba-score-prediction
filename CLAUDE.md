...
- Each research session gets a session_id (`YYYYMMDD_n`) and logs every run to `results/sessions/<session_id>.csv` (`results/sessions/` doesn't exist yet — create on first use), same schema as `experiments_v2.csv` + `session_id`. Experiment IDs are prefixed with the session_id.
