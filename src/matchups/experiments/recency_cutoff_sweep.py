"""
Recency cutoff as an EXPLORED axis, not a hardcoded decision.
...
robustness lens as the Walk-Forward CV Check stage (per-fold, not a single split).
...
config). `hybrid_knn_floor` is not separately swept here: the Walk-Forward CV
Check's fold results showed it produces IDENTICAL output to `wider_exploration_best`
at every fold (the floor never binds at k=81 on this fingerprint), so a recency
sweep on it would be redundant with wider_exploration_best's sweep below.
