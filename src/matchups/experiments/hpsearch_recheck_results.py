...
Context: the wider-exploration run's `tuning.run_optuna_search()` (40 Optuna trials,
TPE, seed=42) found `fingerprint_window=37, decay_halflife=13.2, similarity_method=knn,
knn_k=81, min_confidence_sample=21, full_confidence_sample=82` -- but that search ran
BEFORE the wrap-up round's item #7 fix (the z-score mean/std used to build matchup
vectors were computed globally across the full fingerprint history, a genuine
look-ahead leak). Item #7 fixed this for the walk-forward harness (fitting each fold's
z-score stats on data strictly before that fold's validation_start) and confirmed the
chosen config still wins fold-by-fold -- but explicitly flagged, as a still-open item,
that the SEARCH itself (the single static train/validation split used by
`run_optuna_search`) was never re-run under the corrected normalization: "If the
hyperparameter search were re-run with corrected normalization, the specific best
(window, halflife, k, ...) values might shift slightly." This module closes that gap.
...
windows. Same mechanism as item #7's `zscore_cutoff_date` / walkforward.py's
`zscore_point_in_time`, just applied to this one static split instead of per-fold (see
tuning.py's `run_optuna_search` docstring for the wiring).
...
wider_results.py already established -- method_family/split/fingerprint_window/
decay_halflife columns already exist, so no new columns are introduced this round).
"""
...
def _append_rows_generic(rows: list[dict]) -> None:
    """Column-union append (same pattern as zscore_fix_results.py / item8_results.py):
    any column in `rows` not already in the CSV is added (blank-filled for existing
    rows); any existing column not in `rows` is added to the new rows (blank-filled).
    Deliberately NOT wider_results.py's `_append_rows`, which restricts to a fixed,
...
# The wider-exploration run's winning config (docs/A7_PHASE_LOG.md, Item #1 of that
# run) -- re-evaluated here under CORRECTED (point-in-time) z-score normalization,
# which it was never evaluated under before (item #7 only corrected the walk-forward
# harness, not this static split).
OLD_BEST_HP_CONFIG = {
