...
from src.serving.schedule_lookup import resolve_home_away
...
# Paired home_*/away_* fields that must swap together if the schedule
# disagrees with the model's own home/away guess -- team_id plus every
# market field that's tied to a specific side, not total_line/over_odds/
# under_odds/push_odds, which aren't team-specific.
_HOME_AWAY_FIELD_PAIRS = [
...
def _swap_home_away(pick: dict) -> dict:
    swapped = dict(pick)
    for home_key, away_key in _HOME_AWAY_FIELD_PAIRS:
        home_val, away_val = pick.get(home_key, _MISSING), pick.get(away_key, _MISSING)
        if home_val is _MISSING and away_val is _MISSING:
            continue
        if away_val is _MISSING:
            swapped.pop(home_key, None)
        else:
            swapped[home_key] = away_val
        if home_val is _MISSING:
            swapped.pop(away_key, None)
        else:
            swapped[away_key] = home_val
    return swapped


def _resolve_pick_home_away(pick: dict, game_date: str) -> dict:
    """Overrides the model's home/away guess with the NBA's own published
    schedule when available (see schedule_lookup.py's docstring for why the
    model's guess alone isn't trusted); falls back to the model's guess
    unchanged if no matching scheduled game is found."""
    resolved = resolve_home_away(pick["home_team_id"], pick["away_team_id"], game_date)
    if resolved is None or resolved[0] == pick["home_team_id"]:
        return pick
    return _swap_home_away(pick)
...
Home/away, Hebrew (right-to-left) layouts: the two teams' odds boxes are positioned side by side in \
the image. The box on the PHYSICAL RIGHT side of the screen is always the HOME team, and the box on \
the PHYSICAL LEFT is always the AWAY team -- this is a fixed layout rule of the site, independent of \
which team is favored or which side of the spread/total they're printed under. Use each team's actual \
left/right pixel position in the image, not the text reading order inside a box, to decide this.
...
- home_team_nickname, away_team_nickname (required): the team's canonical NBA nickname, translated \
  ...
  than guessing. Assign home vs. away per the right-side-is-home rule above, not by guessing from \
  context.
...
def extract_picks_from_screenshot(
    image_path: str, model: str = "gemini-2.5-flash", client=None, game_date: str | None = None
) -> list[dict]:
    ...
    Home/away in each pick is then cross-checked against the NBA's own
    published schedule for game_date (default: today) and corrected if the
    model's own guess disagreed -- see schedule_lookup.py's docstring for
    why the model's guess alone isn't trusted.
    ...
    return [_resolve_pick_home_away(p, resolved_date) for p in picks]
